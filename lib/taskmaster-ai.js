...
class TaskmasterAI extends EventEmitter {
  ...
  // Förbättrad metod för problemdetektering
  async detectCodeProblems() {
    const problems = [];
    
    try {
      console.log('🔍 Kör TypeScript check...');
      
      // Kontrollera TypeScript fel med bättre error handling
      try {
        ...
        if (error.stderr || error.stdout) {
          ...
          if (errorLines.length > 0) {
            problems.push({
              ...
              message: `${errorLines.length} TypeScript fel detekterade`,
      ...
      try {
        ...
        if (totalProblems > 0) {
          problems.push({
            ...
            count: totalProblems,
            message: `${totalProblems} ESLint problem (${errorCount} errors, ${warningCount} warnings)`,
            suggestion: 'Kör "npm run lint:fix" för att fixa automatiskt'
          });
        }
        
      } catch (error) {
        // ESLint errors hamnar i stderr men det är OK
        if (error.stdout) {
          try {
            ...
            // Kunde inte parsa JSON, ignorera
          }
        }
      }

      console.log('🔍 Kontrollerar dependencies...');
      ...
      try {
        ...
        if (fs.existsSync(packageJsonPath) && fs.existsSync(nodeModulesPath)) {
          ...
          const missingDeps = Object.keys(allDeps).filter(dep => 
            !installedPackages.includes(dep) && !dep.startsWith('@')
          );
        ...
        console.log('⚠️ Kunde inte kontrollera dependencies:', depError.message);
      }

      console.log(`📊 Totalt ${problems.length} problem detekterade`);
      return problems;
